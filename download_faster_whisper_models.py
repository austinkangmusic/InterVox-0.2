...
def find_or_create_model_dir(start_dir="."):
    """Finds the faster_whisper_models directory, and creates it if it doesn't exist."""
    for root, dirs, files in os.walk(start_dir):
        if "faster_whisper_models" in dirs:
            return os.path.join(root, "faster_whisper_models")
    
    # If the directory is not found, create it
    model_dir_path = os.path.join(start_dir, "faster_whisper_models")
    try:
        os.makedirs(model_dir_path, exist_ok=True)
        return model_dir_path
    except Exception as e:
        print(f"Error creating faster_whisper_models directory: {e}")
        exit(1)
...
def remove_model(log_widget, model_name):
    ...
    model_path = os.path.join(model_dir, f"faster-distil-whisper-{model_name}")
